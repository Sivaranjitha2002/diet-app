...
exports.default = new command_1.default('deploy')
    ...
    const deployTargets = runtime_store_1.default
        ...
        if (!runtime_store_1.default.get('payload.' + deploytarget + '.deploy', false)) {
            (0, logger_1.info)();
            (0, logger_1.labeled)(deploytarget, 'deploy skipped').ERROR();
            return false;
        }
        return true;
    });
    ...
    deployTargets.forEach((deploytarget) => {
        switch (deploytarget) {
            case 'client':
                (0, logger_1.labeled)(deploytarget, 'deploy successful').SUCCESS();
            ...
            case 'apig':
                ...
                break;
            case 'functions':
                runtime_store_1.default
                    .get('payload.' + deploytarget + '.targets', [])
                    .forEach((target) => {
    ...
    if (deployTargets.length > 0) {
        (0, logger_1.success)('Catalyst deploy complete!');
    }
    else {
        (0, logger_1.error)('No components deployed!');
    }
}));
